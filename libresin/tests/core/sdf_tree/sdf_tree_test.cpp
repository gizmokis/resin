...
TEST_F(SDFTreeTest, MaterialsAreProperlyDerivedWhenMaterialSetOrRemoved) {
  ...
  auto& mat3 = tree.add_material(resin::Material(glm::vec3(1.F)));
  ...
  // when
  tree.root().set_material(mat3.material_id());
  ...
  //  3   3 3   3
  //           3 3
  ASSERT_TRUE(tree.root().material_id() == mat3.material_id());
  ASSERT_TRUE(tree.root().ancestor_material_id() == std::nullopt);

  it = tree.root().begin();
  ASSERT_TRUE(tree.node(*it).ancestor_material_id() == mat3.material_id());
  ++it;
  ASSERT_TRUE(tree.node(*it).ancestor_material_id() == mat3.material_id());
